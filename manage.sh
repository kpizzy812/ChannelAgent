#!/bin/bash

# Цвета для оформления
RED='\033[0;31m'
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
BLUE='\033[0;34m'
PURPLE='\033[0;35m'
CYAN='\033[0;36m'
NC='\033[0m' # No Color

# Конфигурация
SERVER="syntra"
REMOTE_PATH="/home/agent"
LOCAL_PATH="."
PROJECT_NAME="ChannelAgent"

# Функция для отображения заголовка
show_header() {
    clear
    echo -e "${CYAN}╔══════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║                  🤖 КАНАЛ АГЕНТ МЕНЕДЖЕР                     ║${NC}"
    echo -e "${CYAN}║                     Панель управления                        ║${NC}"
    echo -e "${CYAN}╚══════════════════════════════════════════════════════════════╝${NC}"
    echo ""
}

# Функция для отображения меню
show_menu() {
    echo -e "${BLUE}Выберите действие:${NC}"
    echo ""
    echo -e "${GREEN}1.${NC} 📤 Синхронизация на сервер"
    echo -e "${GREEN}2.${NC} 🔧 Подготовка окружения"
    echo -e "${GREEN}3.${NC} ▶️  Запуск агента"
    echo -e "${GREEN}4.${NC} ⏹️  Остановка агента"
    echo -e "${GREEN}5.${NC} 📊 Логи в реальном времени"
    echo -e "${GREEN}6.${NC} 📥 Синхронизация с сервера"
    echo -e "${GREEN}7.${NC} 📋 Статус агента"
    echo -e "${GREEN}8.${NC} 💾 Бэкап базы данных"
    echo -e "${GREEN}9.${NC} 🔄 Восстановление БД из бэкапа"
    echo -e "${RED}0.${NC} ❌ Выход"
    echo ""
    echo -ne "${YELLOW}Введите номер: ${NC}"
}

# Функция синхронизации с сервера
sync_from_server() {
    echo -e "\n${BLUE}📥 Синхронизация проекта с сервера...${NC}"
    
    if [ ! -f ".rsyncignore" ]; then
        echo -e "${YELLOW}⚠️  Файл .rsyncignore не найден, создаю стандартный...${NC}"
        create_rsyncignore
    fi
    
    echo -e "${CYAN}Синхронизирую файлы (исключая медиа и конфиги)...${NC}"
    rsync -avz --progress --exclude-from=.rsyncignore ${SERVER}:${REMOTE_PATH}/ ${LOCAL_PATH}/
    
    if [ $? -eq 0 ]; then
        echo -e "${GREEN}✅ Синхронизация завершена успешно!${NC}"
    else
        echo -e "${RED}❌ Ошибка при синхронизации!${NC}"
    fi
    
    read -p "Нажмите Enter для продолжения..."
}

# Функция синхронизации на сервер
sync_to_server() {
    echo -e "\n${BLUE}📤 Синхронизация проекта на сервер...${NC}"
    
    if [ ! -f ".rsyncignore" ]; then
        echo -e "${YELLOW}⚠️  Файл .rsyncignore не найден, создаю стандартный...${NC}"
        create_rsyncignore
    fi
    
    echo -e "${CYAN}Синхронизирую файлы на сервер...${NC}"
    echo -e "${YELLOW}Исключения из .rsyncignore:${NC}"
    grep -E "userbot_session|monitoring_state" .rsyncignore || echo "Нет исключений для сессии!"
    rsync -avz --progress --exclude-from=.rsyncignore ${LOCAL_PATH}/ ${SERVER}:${REMOTE_PATH}/
    
    if [ $? -eq 0 ]; then
        echo -e "${GREEN}✅ Синхронизация на сервер завершена успешно!${NC}"
    else
        echo -e "${RED}❌ Ошибка при синхронизации на сервер!${NC}"
    fi
    
    read -p "Нажмите Enter для продолжения..."
}

# Функция создания .rsyncignore
create_rsyncignore() {
    cat > .rsyncignore << 'EOF'
# Python cache и виртуальные окружения
__pycache__/
*.py[cod]
*$py.class
*.so
.Python
build/
develop-eggs/
dist/
downloads/
eggs/
.eggs/
lib/
lib64/
parts/
sdist/
var/
wheels/
pip-wheel-metadata/
share/python-wheels/
*.egg-info/
.installed.cfg
*.egg
MANIFEST

# Виртуальные окружения
venv/
.venv/
env/
.env/
ENV/
env.bak/
venv.bak/

# Логи
logs/
*.log
*.log.*

# Дополнительные env файлы (основной .env должен синхронизироваться)
.env.*

# ВСЯ папка data/ - серверные данные, НЕ синхронизировать!
# (база данных, сессия UserBot, медиа файлы)
data/

# macOS системные файлы
.DS_Store
.AppleDouble
.LSOverride
._*

# IDE настройки
.idea/
.vscode/
*.swp
*.swo
*~

# Временные файлы
tmp/
temp/
*.tmp
*.bak
*.backup

# Git репозиторий (если есть)
.git/
.gitignore

# Другие исключения
*.pid
core.*
*.core
EOF
}

# Функция подготовки окружения
setup_environment() {
    echo -e "\n${BLUE}🔧 Подготовка окружения...${NC}"
    
    echo -e "${CYAN}Выполняю команды на сервере ${SERVER}...${NC}"
    
    ssh ${SERVER} << 'ENDSSH'
        cd /home/agent

        echo "🔍 Проверка директории проекта..."
        pwd
        ls -la

        echo "📦 Установка python3-venv..."
        apt update > /dev/null 2>&1
        apt install -y python3.12-venv > /dev/null 2>&1

        echo "🐍 Создание виртуального окружения..."
        echo "Удаляю старое окружение если есть..."
        rm -rf .venv
        python3 -m venv .venv

        if [ -f ".venv/bin/activate" ]; then
            echo "✅ Виртуальное окружение создано успешно"
        else
            echo "❌ Ошибка создания виртуального окружения"
            exit 1
        fi

        echo "📦 Установка зависимостей..."
        source .venv/bin/activate
        pip install --upgrade pip
        
        if [ -f "requirements.txt" ]; then
            pip install -r requirements.txt
            echo "✅ Зависимости установлены"
        else
            echo "⚠️  Файл requirements.txt не найден"
        fi
        
        echo "📁 Создание необходимых директорий..."
        mkdir -p data/media/photos data/media/videos data/media/thumbnails logs
        
        echo "🔧 Подготовка завершена!"
ENDSSH
    
    if [ $? -eq 0 ]; then
        echo -e "${GREEN}✅ Подготовка окружения завершена успешно!${NC}"
    else
        echo -e "${RED}❌ Ошибка при подготовке окружения!${NC}"
    fi
    
    read -p "Нажмите Enter для продолжения..."
}

# Функция запуска агента
start_agent() {
    echo -e "\n${BLUE}▶️  Запуск агента...${NC}"
    
    echo -e "${CYAN}Запускаю агента на сервере ${SERVER}...${NC}"
    
    ssh ${SERVER} << 'ENDSSH'
        cd /home/agent
        
        echo "🔍 Проверка процессов агента..."
        if pgrep -f "python.*main.py" > /dev/null; then
            echo "⚠️  Агент уже запущен!"
            echo "Запущенные процессы:"
            pgrep -f "python.*main.py" | while read pid; do
                echo "PID: $pid - $(ps -p $pid -o args --no-headers)"
            done
            exit 1
        fi
        
        echo "🚀 Запуск агента..."

        # Проверка наличия виртуального окружения
        if [ ! -f ".venv/bin/activate" ]; then
            echo "❌ Виртуальное окружение не найдено! Сначала выполните 'Подготовка окружения'"
            exit 1
        fi

        source .venv/bin/activate

        # Запуск в фоне с перенаправлением логов
        nohup python3 main.py > agent.log 2>&1 &
        AGENT_PID=$!
        
        echo "✅ Агент запущен с PID: $AGENT_PID"
        echo $AGENT_PID > agent.pid
        
        # Ждем немного и проверяем что процесс действительно запустился
        sleep 3
        if ps -p $AGENT_PID > /dev/null; then
            echo "✅ Агент успешно запущен и работает"
        else
            echo "❌ Ошибка: агент не смог запуститься"
            echo "Последние строки лога:"
            tail -10 agent.log
        fi
ENDSSH
    
    if [ $? -eq 0 ]; then
        echo -e "${GREEN}✅ Агент запущен успешно!${NC}"
    else
        echo -e "${RED}❌ Ошибка при запуске агента!${NC}"
    fi
    
    read -p "Нажмите Enter для продолжения..."
}

# Функция остановки агента
stop_agent() {
    echo -e "\n${BLUE}⏹️  Остановка агента...${NC}"
    
    echo -e "${CYAN}Останавливаю агента на сервере ${SERVER}...${NC}"
    
    ssh ${SERVER} << 'ENDSSH'
        cd /home/agent
        
        echo "🔍 Поиск процессов агента..."
        PIDS=$(pgrep -f "python.*main.py")
        
        if [ -z "$PIDS" ]; then
            echo "ℹ️  Процессы агента не найдены"
            # Удаляем pid файл если он есть
            rm -f agent.pid
        else
            echo "🛑 Останавливаю процессы:"
            echo "$PIDS" | while read pid; do
                echo "Останавливаю PID: $pid - $(ps -p $pid -o args --no-headers)"
                kill $pid
            done
            
            # Ждем немного для graceful shutdown
            sleep 3
            
            # Проверяем что процессы остановились
            REMAINING=$(pgrep -f "python.*main.py")
            if [ ! -z "$REMAINING" ]; then
                echo "⚠️  Принудительная остановка оставшихся процессов..."
                echo "$REMAINING" | while read pid; do
                    kill -9 $pid
                done
            fi
            
            rm -f agent.pid
            echo "✅ Все процессы агента остановлены"
        fi
ENDSSH
    
    if [ $? -eq 0 ]; then
        echo -e "${GREEN}✅ Агент остановлен успешно!${NC}"
    else
        echo -e "${RED}❌ Ошибка при остановке агента!${NC}"
    fi
    
    read -p "Нажмите Enter для продолжения..."
}

# Функция просмотра логов
view_logs() {
    echo -e "\n${BLUE}📊 Логи в реальном времени (последние 500 записей)...${NC}"
    echo -e "${YELLOW}Нажмите Ctrl+C для выхода${NC}"
    echo ""

    ssh ${SERVER} "cd /home/agent && tail -n 500 -f agent.log"
}

# Функция проверки статуса
check_status() {
    echo -e "\n${BLUE}📋 Статус агента...${NC}"
    
    ssh ${SERVER} << 'ENDSSH'
        cd /home/agent
        
        echo "🔍 Проверка процессов агента..."
        PIDS=$(pgrep -f "python.*main.py")
        
        if [ -z "$PIDS" ]; then
            echo "❌ Агент не запущен"
        else
            echo "✅ Агент запущен:"
            echo "$PIDS" | while read pid; do
                echo "  PID: $pid - $(ps -p $pid -o args --no-headers)"
                echo "  Время работы: $(ps -p $pid -o etime --no-headers | tr -d ' ')"
                echo "  Использование CPU: $(ps -p $pid -o %cpu --no-headers | tr -d ' ')%"
                echo "  Использование памяти: $(ps -p $pid -o %mem --no-headers | tr -d ' ')%"
            done
        fi
        
        echo ""
        echo "📁 Размер лог файла:"
        if [ -f "agent.log" ]; then
            ls -lh agent.log | awk '{print "  " $5 " (" $9 ")"}'
            echo ""
            echo "📝 Последние 5 строк лога:"
            tail -5 agent.log | sed 's/^/  /'
        else
            echo "  Лог файл не найден"
        fi
        
        echo ""
        echo "💾 Использование диска:"
        df -h . | tail -1 | awk '{print "  Использовано: " $3 " из " $2 " (" $5 ")"}'
ENDSSH
    
    read -p "Нажмите Enter для продолжения..."
}

# Функция бэкапа базы данных
backup_database() {
    echo -e "\n${BLUE}💾 Бэкап базы данных...${NC}"

    # Создаем локальную директорию для бэкапов
    LOCAL_BACKUP_DIR="./backups"
    mkdir -p ${LOCAL_BACKUP_DIR}

    # Формируем имя файла с датой и временем
    TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
    BACKUP_NAME="agent_db_${TIMESTAMP}.sqlite"
    REMOTE_DB_PATH="/home/agent/data/channel_agent.db"

    echo -e "${CYAN}📦 Создаю бэкап на сервере...${NC}"

    # Создаем бэкап на сервере (с блокировкой для консистентности)
    ssh ${SERVER} << ENDSSH
        cd /home/agent

        if [ ! -f "${REMOTE_DB_PATH}" ]; then
            echo "❌ База данных не найдена: ${REMOTE_DB_PATH}"
            exit 1
        fi

        # Получаем размер БД
        DB_SIZE=\$(du -h ${REMOTE_DB_PATH} | cut -f1)
        echo "📊 Размер БД: \${DB_SIZE}"

        # Создаем временный бэкап с использованием sqlite3 для консистентности
        if command -v sqlite3 &> /dev/null; then
            echo "🔒 Создаю консистентный бэкап через sqlite3..."
            sqlite3 ${REMOTE_DB_PATH} ".backup /tmp/${BACKUP_NAME}"
        else
            echo "📋 Копирую файл напрямую..."
            cp ${REMOTE_DB_PATH} /tmp/${BACKUP_NAME}
        fi

        echo "✅ Бэкап создан: /tmp/${BACKUP_NAME}"
ENDSSH

    if [ $? -ne 0 ]; then
        echo -e "${RED}❌ Ошибка создания бэкапа на сервере!${NC}"
        read -p "Нажмите Enter для продолжения..."
        return
    fi

    echo -e "${CYAN}📥 Скачиваю бэкап на локальную машину...${NC}"

    # Скачиваем бэкап
    scp ${SERVER}:/tmp/${BACKUP_NAME} ${LOCAL_BACKUP_DIR}/${BACKUP_NAME}

    if [ $? -eq 0 ]; then
        # Получаем размер скачанного файла
        LOCAL_SIZE=$(du -h ${LOCAL_BACKUP_DIR}/${BACKUP_NAME} | cut -f1)

        echo -e "${GREEN}✅ Бэкап успешно сохранен!${NC}"
        echo -e "${CYAN}📁 Путь: ${LOCAL_BACKUP_DIR}/${BACKUP_NAME}${NC}"
        echo -e "${CYAN}📊 Размер: ${LOCAL_SIZE}${NC}"

        # Удаляем временный файл на сервере
        ssh ${SERVER} "rm -f /tmp/${BACKUP_NAME}"

        # Показываем список последних бэкапов
        echo -e "\n${BLUE}📋 Последние бэкапы:${NC}"
        ls -lh ${LOCAL_BACKUP_DIR}/*.sqlite 2>/dev/null | tail -5 | while read line; do
            echo -e "  ${line}"
        done

        # Считаем общее количество бэкапов
        BACKUP_COUNT=$(ls -1 ${LOCAL_BACKUP_DIR}/*.sqlite 2>/dev/null | wc -l | tr -d ' ')
        echo -e "\n${YELLOW}💡 Всего бэкапов: ${BACKUP_COUNT}${NC}"

        # Предупреждаем если много бэкапов
        if [ ${BACKUP_COUNT} -gt 10 ]; then
            echo -e "${YELLOW}⚠️  Рекомендуется удалить старые бэкапы для экономии места${NC}"
        fi
    else
        echo -e "${RED}❌ Ошибка скачивания бэкапа!${NC}"
    fi

    read -p "Нажмите Enter для продолжения..."
}

# Функция восстановления базы данных из бэкапа
restore_database() {
    echo -e "\n${BLUE}🔄 Восстановление базы данных из бэкапа...${NC}"

    LOCAL_BACKUP_DIR="./backups"
    REMOTE_DB_PATH="/home/agent/data/channel_agent.db"

    # Проверяем наличие бэкапов
    if [ ! -d "${LOCAL_BACKUP_DIR}" ] || [ -z "$(ls -A ${LOCAL_BACKUP_DIR}/*.sqlite 2>/dev/null)" ]; then
        echo -e "${RED}❌ Бэкапы не найдены в ${LOCAL_BACKUP_DIR}${NC}"
        echo -e "${YELLOW}💡 Сначала создайте бэкап (пункт 8)${NC}"
        read -p "Нажмите Enter для продолжения..."
        return
    fi

    # Показываем список бэкапов
    echo -e "${CYAN}📋 Доступные бэкапы:${NC}"
    echo ""

    # Создаем массив файлов
    BACKUPS=()
    i=1
    for backup in $(ls -t ${LOCAL_BACKUP_DIR}/*.sqlite 2>/dev/null); do
        BACKUPS+=("$backup")
        FILENAME=$(basename "$backup")
        SIZE=$(du -h "$backup" | cut -f1)
        DATE=$(echo "$FILENAME" | sed 's/agent_db_\([0-9]*\)_\([0-9]*\).sqlite/\1 \2/' | sed 's/\([0-9]\{4\}\)\([0-9]\{2\}\)\([0-9]\{2\}\) \([0-9]\{2\}\)\([0-9]\{2\}\)\([0-9]\{2\}\)/\1-\2-\3 \4:\5:\6/')
        echo -e "${GREEN}${i}.${NC} ${FILENAME} (${SIZE}) - ${DATE}"
        ((i++))
    done

    echo -e "${RED}0.${NC} Отмена"
    echo ""
    echo -ne "${YELLOW}Выберите бэкап для восстановления: ${NC}"
    read -r choice

    if [ "$choice" == "0" ] || [ -z "$choice" ]; then
        echo -e "${YELLOW}Отмена операции${NC}"
        read -p "Нажмите Enter для продолжения..."
        return
    fi

    # Проверяем корректность выбора
    if ! [[ "$choice" =~ ^[0-9]+$ ]] || [ "$choice" -lt 1 ] || [ "$choice" -gt ${#BACKUPS[@]} ]; then
        echo -e "${RED}❌ Неверный выбор${NC}"
        read -p "Нажмите Enter для продолжения..."
        return
    fi

    SELECTED_BACKUP="${BACKUPS[$((choice-1))]}"
    BACKUP_FILENAME=$(basename "$SELECTED_BACKUP")

    echo ""
    echo -e "${YELLOW}⚠️  ВНИМАНИЕ! Эта операция:${NC}"
    echo -e "  • Остановит агента"
    echo -e "  • Создаст бэкап текущей БД"
    echo -e "  • Заменит БД на: ${BACKUP_FILENAME}"
    echo -e "  • Перезапустит агента"
    echo ""
    echo -ne "${RED}Вы уверены? (да/нет): ${NC}"
    read -r confirm

    if [ "$confirm" != "да" ] && [ "$confirm" != "yes" ] && [ "$confirm" != "y" ]; then
        echo -e "${YELLOW}Отмена операции${NC}"
        read -p "Нажмите Enter для продолжения..."
        return
    fi

    echo ""
    echo -e "${CYAN}🔄 Начинаю восстановление...${NC}"

    # 1. Остановка агента
    echo -e "${CYAN}⏹️  Останавливаю агента...${NC}"
    ssh ${SERVER} << 'ENDSSH'
        cd /home/agent
        PIDS=$(pgrep -f "python.*main.py")
        if [ ! -z "$PIDS" ]; then
            echo "$PIDS" | while read pid; do
                kill $pid 2>/dev/null
            done
            sleep 3
            # Force kill если не остановился
            REMAINING=$(pgrep -f "python.*main.py")
            if [ ! -z "$REMAINING" ]; then
                echo "$REMAINING" | while read pid; do
                    kill -9 $pid 2>/dev/null
                done
            fi
        fi
        rm -f agent.pid
ENDSSH

    # 2. Создаем бэкап текущей БД на сервере
    echo -e "${CYAN}💾 Создаю бэкап текущей БД перед заменой...${NC}"
    TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
    ssh ${SERVER} "cp ${REMOTE_DB_PATH} ${REMOTE_DB_PATH}.before_restore_${TIMESTAMP} 2>/dev/null || echo 'БД не существует'"

    # 3. Загружаем бэкап на сервер
    echo -e "${CYAN}📤 Загружаю бэкап на сервер...${NC}"
    scp "${SELECTED_BACKUP}" ${SERVER}:/tmp/restore_db.sqlite

    if [ $? -ne 0 ]; then
        echo -e "${RED}❌ Ошибка загрузки бэкапа на сервер!${NC}"
        read -p "Нажмите Enter для продолжения..."
        return
    fi

    # 4. Заменяем БД
    echo -e "${CYAN}🔄 Заменяю базу данных...${NC}"
    ssh ${SERVER} << ENDSSH
        cd /home/agent
        mkdir -p data
        mv /tmp/restore_db.sqlite ${REMOTE_DB_PATH}
        chmod 644 ${REMOTE_DB_PATH}
        echo "✅ База данных заменена"
        ls -lh ${REMOTE_DB_PATH}
ENDSSH

    if [ $? -ne 0 ]; then
        echo -e "${RED}❌ Ошибка замены БД!${NC}"
        read -p "Нажмите Enter для продолжения..."
        return
    fi

    # 5. Запрос на перезапуск агента
    echo ""
    echo -ne "${YELLOW}Запустить агента? (да/нет): ${NC}"
    read -r start_agent

    if [ "$start_agent" == "да" ] || [ "$start_agent" == "yes" ] || [ "$start_agent" == "y" ]; then
        echo -e "${CYAN}▶️  Запускаю агента...${NC}"
        ssh ${SERVER} << 'ENDSSH'
            cd /home/agent
            source .venv/bin/activate
            nohup python3 main.py > agent.log 2>&1 &
            AGENT_PID=$!
            echo $AGENT_PID > agent.pid
            sleep 3
            if ps -p $AGENT_PID > /dev/null; then
                echo "✅ Агент запущен с PID: $AGENT_PID"
            else
                echo "❌ Ошибка запуска агента"
                tail -10 agent.log
            fi
ENDSSH
    fi

    echo ""
    echo -e "${GREEN}✅ Восстановление завершено!${NC}"
    echo -e "${CYAN}📁 Использован бэкап: ${BACKUP_FILENAME}${NC}"
    echo -e "${YELLOW}💡 Старая БД сохранена как: channel_agent.db.before_restore_${TIMESTAMP}${NC}"

    read -p "Нажмите Enter для продолжения..."
}

# Главный цикл программы
main() {
    while true; do
        show_header
        show_menu
        read -r choice
        
        case $choice in
            1)
                sync_to_server
                ;;
            2)
                setup_environment
                ;;
            3)
                start_agent
                ;;
            4)
                stop_agent
                ;;
            5)
                view_logs
                ;;
            6)
                sync_from_server
                ;;
            7)
                check_status
                ;;
            8)
                backup_database
                ;;
            9)
                restore_database
                ;;
            0)
                echo -e "\n${GREEN}👋 До свидания!${NC}"
                exit 0
                ;;
            *)
                echo -e "\n${RED}❌ Неверный выбор. Пожалуйста, выберите число от 0 до 9.${NC}"
                sleep 2
                ;;
        esac
    done
}

# Проверка наличия SSH подключения
check_ssh_connection() {
    echo -e "${CYAN}🔍 Проверка подключения к серверу ${SERVER}...${NC}"
    if ssh -o ConnectTimeout=5 ${SERVER} "echo 'Подключение успешно'" > /dev/null 2>&1; then
        echo -e "${GREEN}✅ Подключение к серверу установлено${NC}"
        return 0
    else
        echo -e "${RED}❌ Не удается подключиться к серверу ${SERVER}${NC}"
        echo -e "${YELLOW}Убедитесь что:${NC}"
        echo -e "  • SSH ключ настроен"
        echo -e "  • Алиас 'syntra' существует в ~/.ssh/config"
        echo -e "  • Сервер доступен"
        exit 1
    fi
}

# Запуск программы
echo -e "${CYAN}Инициализация Канал Агент Менеджера...${NC}"
check_ssh_connection
sleep 1
main